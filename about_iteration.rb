...
class AboutIteration < Neo::Koan
  ...
  def test_each_can_use_curly_brace_blocks_too
    array = [1, 2, 3]
    sum = 0
    array.each { |whatevername| sum += whatevername }
    assert_equal 6, sum
  end
  ...
  def test_inject_will_blow_your_mind
    ...
    # Inject does an operation on each item within the array. The operation can be defined in arguments or the following scope.
  end
